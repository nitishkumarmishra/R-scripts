#IntegrationCNVandExpression.R
########## R program to calculate Spearman's correlation ###########
## This program select only genes which have at least CN gain/loss in 10% samples
# Between TCGA level-4 CNV and Level-3 gene expression data, p-value and BH adjusted p-value 0.01
####################################################################
library(matrixStats)
library(multtest)
library(impute)
...
PAAD.uncv2.mRNAseq_RSEM.common <- PAAD.uncv2.mRNAseq_RSEM[c(x1),c(x)]
all_data.by_genes.common <- all_data.by_genes[c(x1),c(x)]
#Expr.common <- as.matrix(PAAD.uncv2.mRNAseq_RSEM.common)
PAAD.uncv2.mRNAseq_RSEM.common.impute <- impute.knn(PAAD.uncv2.mRNAseq_RSEM.common, k = 15, rowmax = 0.8, colmax = 0.8)
PAAD.uncv2.mRNAseq_RSEM.common.impute <- PAAD.uncv2.mRNAseq_RSEM.common.impute$data
all_data.by_genes.common <- all_data.by_genes[c(x1),c(x)]
all_thresholded.by_genes.common <- all_thresholded.by_genes[c(x1),c(x)]
CNV.common <- as.matrix(all_data.by_genes.common)
# CNV.loss <- apply(all_data.by_genes.common,1, function(x) which(x <= -0.10))
# CNV.gain <- apply(all_data.by_genes.common,1, function(x) which(x >= 0.10))
#CNV.common <- as.matrix(all_thresholded.by_genes.common)
...
for(i in 1:nrow(CNV.common))
{
  ...
  a2[i,a] <- PAAD.uncv2.mRNAseq_RSEM.common.impute[i,a]
}
...
adjust.order.1 <- data.frame(adjust.order)
final.CNVcommon <- cbind(CNVcommon.p, adjust.order.1)
CNVgain.adjP.0.05 <- final.CNVcommon[(final.CNVcommon$rawp <= 0.01) & (final.CNVcommon$Bonferroni <= 0.05),]
dim(CNVgain.adjP.0.05)
CNVgain.adjP.0.05[order(rownames(CNVgain.adjP.0.05), decreasing = FALSE),]### Order by Gene name
CNVgain.adjP.0.05[order(CNVgain.adjP.0.05[,4], decreasing = FALSE),]#
rm(ll, cor.value, p.value, CNVcommon.p, adjust, adjust.order, adjust.order.1)
#######################################################
