...
rm(a, i, j, tmp, tmp1, tmpRow, x3, sampleIDs1, samplesDat, sampleIDs, colName, tumorSamples, normalSamples, 
   a1, a1.1, a1.1.common, t, names.less17, x, x1, a2, a2.1, a2.1.common)
#################################################################
...
rm(a1.1.common.1, a2.1.common.1, all_data.by_genes, all_data.by_genes.common, all_thresholded.by_genes, all_thresholded.by_genes.common, CNV.common, CNV.gain, CNV.loss, Expr.common, final.CNVcommon, 
   m1.1.common.1, m2.1.common.1, normalSamples.Expression, tumorSamples.Expression, PAAD.uncv2.mRNAseq_RSEM, PAAD.uncv2.mRNAseq_RSEM_normalized_log2,ll, cor.value, p.value, CNVcommon.p, adjust, adjust.order, adjust.order.1)
#######################################################
#################### Welch t-test #####################
source("Welch-t-test-CNV.R")

####### Final list of genes in both data set ##########
Welch.gain <- Welch.CNVgain.adjP.0.05[, c(2,3,4)]
Welch.loss <- Welch.CNVloss.adjP.0.05[, c(2,3,4)]
tmp <- intersect(rownames(Welch.gain), rownames(CNVgain.adjP.0.01))
CNVgain.adjP.0.01.Welch <- cbind(CNVgain.adjP.0.01[tmp,],Welch.gain[tmp,])
tmp <- intersect(rownames(Welch.loss), rownames(CNVloss.adjP.0.01))
CNVloss.adjP.0.01.Welch <- cbind(CNVloss.adjP.0.01[tmp,],Welch.loss[tmp,])
rm(tmp)