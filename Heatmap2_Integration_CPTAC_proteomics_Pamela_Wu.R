...
genes <- c("UBE2T", "BIRC5", "NUF2", "CDC6", "CCNB1", "TYMS", "MYBL2", "CEP55", "MELK",
           ...
           "NAT1", "FOXA1", "BLVRA", "MMP11", "GPR160", "FGFR4", "GRB7", "TMEM45B", "ERBB2")
...
# fetch TCGA subtypes
#https://tcga-data.nci.nih.gov/docs/publications/brca_2012/BRCA.547.PAM50.SigClust.Subtypes.txt
## Download and save file from above URL
#subtypes <- read.table("BRCA.547.PAM50.SigClust.Subtypes.txt", sep="\t", header=TRUE, colClasses="character")
# Rather than we can use this command to directly download CVS file from given URL
subtypes <- data.table::fread('https://tcga-data.nci.nih.gov/docs/publications/brca_2012/BRCA.547.PAM50.SigClust.Subtypes.txt')
